...
results = criticality_prg_analysis(dataStruct, analysisConfig);

if ~isempty(brainArea)
  results = filter_prg_results_to_brain_area(results, brainArea);
  if isempty(results.areas)
    error('No PRG results for brain area "%s".', brainArea);
  end
end
...
criticality_prg_plot(results, plotConfig, analysisConfig, dataStruct);
