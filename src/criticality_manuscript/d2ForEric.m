...
% Output container
d2Results = struct();
d2Results.readme = build_d2_for_eric_readme();
...
function readme = build_d2_for_eric_readme()
% BUILD_D2_FOR_ERIC_README - Text describing the saved d2ForEric struct
%
% Goal:
%   Provide a human-readable field documenting how to navigate the .mat
%   contents after load().
%
% Returns:
%   readme - Cell array of char lines (display with fprintf('%s\n', readme{:}))

readme = {
  'd2ForEric — sliding-window d2 results for reach sessions'
  ''
  'Load:'
  '  S = load(''d2Results.mat'');  % or whatever path was used'
  '  d2ForEric = S.d2ForEric;'
  '  disp(d2ForEric.readme)      % this document'
  ''
  'Top-level fields'
  '  .readme            This documentation (cellstr).'
  '  .windowsToTest     Vector of d2 window lengths tested (seconds).'
  '  .binSizesToTest    Vector of spike bin widths tested (seconds).'
  '  .stepSize          Sliding-window step / hop (seconds); same for all runs.'
  '  .brainArea         Population / merged area analyzed (e.g. ''M2356'').'
  '  .collectStart      Session analysis start time (s from session onset).'
  '  .collectEnd        Session analysis end time (s); [] = full loaded session.'
  '  .useLog10D2        If true, stored .d2 values are log10(d2); non-positive -> NaN.'
  '  .sessions          Struct array, one entry per reach session.'
  ''
  'Per-session fields: d2ForEric.sessions(i)'
  '  .sessionName       Reach session identifier string.'
  '  .success           True if at least one parameter run succeeded.'
  '  .skipReason        Empty on success; otherwise why the session failed/skipped.'
  '  .runs              Struct array of parameter-grid runs for this session.'
  ''
  'Per-run fields: d2ForEric.sessions(i).runs(j)'
  '  Runs are ordered by nested loops: window size outermost, then bin size.'
  '  Index j = (iWin-1)*numel(binSizesToTest) + iBin.'
  ''
  '  .windowSize        Sliding d2 window length (seconds).'
  '  .binSize           Spike bin width used for this run (seconds).'
  '  .stepSize          Window step (seconds); equals top-level .stepSize.'
  '  .timestamps        Column vector of window-center times (absolute session'
  '                     time in seconds). Aligned 1:1 with .d2.'
  '  .d2                Column vector of d2 (or log10(d2) if useLog10D2) per window.'
  '  .areaName          Brain area name for this run (same as top-level brainArea).'
  '  .success           True if this window/bin combination produced valid d2.'
  '  .skipReason        Empty on success; otherwise failure / skip message.'
  ''
  'Notes'
  '  - Timestamps are window centers from criticality_ar_analysis (startS).'
  '  - Failed runs keep windowSize/binSize/stepSize filled but empty d2/timestamps.'
  '  - Generated by criticality_manuscript/d2ForEric.m'
  };
end
