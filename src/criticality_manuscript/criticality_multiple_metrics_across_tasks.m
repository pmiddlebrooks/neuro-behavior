...
if pcaFlag
  ...
  fprintf('PCA reconstruction: off\n');
end
  ...
  if ~splitByEngagement
    ...
    if ~isempty(metricsToPlot)
      plot_multimetric_d2_tau_alpha_across_tasks(arOut.plotData, avOut.plotData, plotAreas, ...
        ...
        useSubsampling, nNeuronsSubsample, avWindow, pcaFileTag);
    end
    if plotSeparatedMetrics
      plot_multimetric_separated_axes_across_tasks(arOut.plotData, avOut.plotData, ...
        ...
        activeAnalyses, avWindow, binSizeD2, pcaFileTag);
    end
    if plotMetricPairScatters
      plot_multimetric_pair_scatters_across_tasks(arOut.plotData, avOut.plotData, plotAreas, ...
        sessionTypes, collectStart, collectEnd, d2Window, paths, brainArea, useLog10D2, ...
        plotConfig, '', useSubsampling, nNeuronsSubsample, activeAnalyses, avWindow, pcaFileTag);
    end
  else
    ...
    % Shared maps + y-limits across engaged/non-engaged so sister axes match
    if ~isempty(metricsToPlot)
    ...
    sharedSeparatedYLim = struct();
    if plotSeparatedMetrics
      sharedSeparatedYLim = compute_shared_separated_engagement_ylims( ...
        classViews, plotAreas, sessionTypes, metricsToPlot, enablePermutations, activeAnalyses);
    end
    ...
    for iClass = 1:numel(engagementClasses)
      ...
      if plotSeparatedMetrics
        plot_multimetric_separated_axes_across_tasks( ...
          ...
          finalCutoffDivisor, enablePermutations, useSubsampling, nNeuronsSubsample, ...
          activeAnalyses, avWindow, binSizeD2, pcaFileTag, sharedSeparatedYLim);
      end
      if plotMetricPairScatters
        plot_multimetric_pair_scatters_across_tasks( ...
          classViews.(engClass).ar, classViews.(engClass).av, plotAreas, ...
          sessionTypes, collectStart, collectEnd, d2Window, paths, brainArea, useLog10D2, ...
          plotConfig, engClass, useSubsampling, nNeuronsSubsample, activeAnalyses, avWindow, pcaFileTag);
      end
    end
  end
else
...
if plotCorrelationMatrix
  ...
  plot_metric_correlation_matrix_across_sessions( ...
    ...
    activeAnalyses, avWindow, pcaFileTag);
end
...
function plot_metric_correlation_matrix_across_sessions(arOut, avOut, prgOut, areasToPlot, ...
    ...
    useSubsampling, nNeuronsSubsample, activeAnalyses, avWindow, pcaFileTag)
...
if nargin < 17 || isempty(pcaFileTag)
  pcaFileTag = '';
end
...
if nargin < 10 || isempty(pcaFileTag)
  pcaFileTag = '';
end
...
plotBase = sprintf('metric_corr_across_sessions_%s_win%s_av%s_%s%s%s%s', ...
  areaTag, winTag, avTag, timeTag, logTag, ...
  format_subsamp_file_tag(useSubsampling, nNeuronsSubsample), char(pcaFileTag));
end
...
if nargin < 20 || isempty(pcaFileTag)
  pcaFileTag = '';
end
metricsToPlot = normalize_metrics_to_plot(metricsToPlot);
...
function plot_multimetric_separated_axes_across_tasks(arPlotData, avPlotData, prgPlotData, ...
    ...
    avWindow, binSizeD2, pcaFileTag, sharedYLimByArea)
...
%   sharedYLimByArea        - Optional struct areaKey.metricKey = [ymin ymax]
%                             for engaged/non-engaged sister-panel matching
%
% Goal:
...
if nargin < 23 || isempty(pcaFileTag)
  pcaFileTag = '';
end
if nargin < 24 || isempty(sharedYLimByArea)
  sharedYLimByArea = struct();
end
activeAnalyses = fill_active_analyses(activeAnalyses);
...
for a = 1:numel(areasToPlot)
  ...
  for iPanel = 1:nPanels
    ...
    yLimPlot = [];
    areaKey = matlab.lang.makeValidName(areaName);
    if isstruct(sharedYLimByArea) && isfield(sharedYLimByArea, areaKey) ...
        && isfield(sharedYLimByArea.(areaKey), metricKey)
      yLimPlot = sharedYLimByArea.(areaKey).(metricKey);
    end
    if isempty(yLimPlot)
...
if nargin < 12 || isempty(pcaFileTag)
  pcaFileTag = '';
end
collectTag = format_multimetric_collect_tag(collectStart, collectEnd);
winTag = format_window_sec_file_tag(d2Window);
...
plotBase = [plotBase, format_subsamp_file_tag(useSubsampling, nNeuronsSubsample), char(pcaFileTag)];
end
...
if nargin < 14 || isempty(pcaFileTag)
  pcaFileTag = '';
end
metricsToPlot = normalize_metrics_to_plot(metricsToPlot);
collectTag = format_multimetric_collect_tag(collectStart, collectEnd);
...
if nargin < 11 || isempty(pcaFileTag)
  pcaFileTag = '';
end
collectTag = format_multimetric_collect_tag(collectStart, collectEnd);
winTag = format_window_sec_file_tag(d2Window);
...
function titleStr = append_pca_title_tag(titleStr, pcaFileTag)
% APPEND_PCA_TITLE_TAG - Append pca_nDim (or pcaLast_nDim) when PCA reconstruction is on
if nargin < 2 || isempty(pcaFileTag)
  return;
end
pcaFileTag = char(pcaFileTag);
if pcaFileTag(1) == '_'
  pcaFileTag = pcaFileTag(2:end);
end
...
function sharedYLimByArea = compute_shared_separated_engagement_ylims( ...
    classViews, plotAreas, sessionTypes, metricsToPlot, enablePermutations, activeAnalyses)
% COMPUTE_SHARED_SEPARATED_ENGAGEMENT_YLIMS Sister-panel ylims for engaged/non-engaged
%
% Variables:
%   classViews         - Struct with .engaged / .nonEngaged .ar/.av/.prg plotData
%   plotAreas          - Cell of area names present in both classes
%   sessionTypes       - Session types on the x-axis
%   metricsToPlot      - Active d2/tau/alpha subset (empty = pipeline default)
%   enablePermutations - If true, include shuffle/surrogate points in ylims
%   activeAnalyses     - Struct .ar/.av/.prg
%
% Goal:
%   One y-limit per area and metric so the engaged and non-engaged separated
%   metrics figures can be compared on the same scale.
...
sharedYLimByArea = struct();
if isempty(activePanelKeys) || isempty(plotAreas) || isempty(classViews)
  return;
end
engClasses = fieldnames(classViews);
...
for a = 1:numel(plotAreas)
  areaName = plotAreas{a};
  areaKey = matlab.lang.makeValidName(areaName);
  pooledByMetric = struct();
  for iKey = 1:numel(activePanelKeys)
    pooledByMetric.(activePanelKeys{iKey}) = [];
  end
  ...
  for iClass = 1:numel(engClasses)
    ...
    arPlotData = classViews.(engClass).ar;
    avPlotData = classViews.(engClass).av;
    prgPlotData = classViews.(engClass).prg;
    areaIdxAr = find(strcmp(arPlotData.areas, areaName), 1);
    areaIdxAv = find(strcmp(avPlotData.areas, areaName), 1);
    areaIdxPrg = find(strcmp(prgPlotData.areas, areaName), 1);
    if isempty(areaIdxAr), areaIdxAr = 1; end
    ...
    keepBlankSessions = strcmpi(engClass, 'nonEngaged') || numel(activePanelKeys) < 8;
    sessionTable = build_separated_metrics_session_table( ...
      arPlotData, avPlotData, prgPlotData, avPlotData, sessionTypes, ...
      areaIdxAr, areaIdxAv, areaIdxPrg, areaIdxAv, topMetrics, keepBlankSessions);
    if isempty(sessionTable)
      continue;
    end

    for iKey = 1:numel(activePanelKeys)
      metricKey = activePanelKeys{iKey};
      yField = fieldByKey.(metricKey);
      yMean = sessionTable.(yField)(:);
      yVals = yMean;
      semField = semByKey.(metricKey);
      if ismember(semField, sessionTable.Properties.VariableNames)
        ySem = sessionTable.(semField)(:);
        yVals = [yVals; yMean + ySem; yMean - ySem]; %#ok<AGROW>
      end
      shuffleField = shuffleFieldByKey.(metricKey);
      if enablePermutations && ismember(shuffleField, sessionTable.Properties.VariableNames)
        yVals = [yVals; sessionTable.(shuffleField)(:)]; %#ok<AGROW>
      end
      pooledByMetric.(metricKey) = [pooledByMetric.(metricKey); yVals]; %#ok<AGROW>
    end
  end

  for iKey = 1:numel(activePanelKeys)
    metricKey = activePanelKeys{iKey};
    yLimPlot = compute_native_ylim_for_metric(pooledByMetric.(metricKey));
    if ~isempty(yLimPlot)
      sharedYLimByArea.(areaKey).(metricKey) = yLimPlot;
    end
  end
end
end
...
engModOpts.pcaFlag = pcaOpts.pcaFlag;
engModOpts.pcaFirstFlag = pcaOpts.pcaFirstFlag;
engModOpts.nDim = pcaOpts.nDim;
