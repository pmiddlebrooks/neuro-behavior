...
classCounts = zeros(size(uniqueLabels));
...
for iBhv = 1:length(uniqueLabels)
    ...
    % Store class counts for weighted F1 computation
    classCounts(iBhv) = sum(svmID == classLabel);
end

% Compute Macro F1 (average of per-class F1 scores)
macroF1 = mean(f1Score);

% Compute Weighted F1 (accounts for class imbalance)
weightedF1 = sum(f1Score .* (classCounts / sum(classCounts)));
