...
data = overlap{1,1}; %pull out raw data
...
figure(55); clf;
plot(movmean(cor,[4 0]),'k','LineWidth',2); %plot the moving average for correct
...
for j = 1:length(wantedEM) %going through all values in wantedEM
    if (wantedEM(j) == 1) || (wantedEM(j) == 3) % LR value
        line([j,j],[1,1.05],'Color',colorNonPref,'LineWidth',widthR); %LR line
    elseif (wantedEM(j) == 2) || (wantedEM(j) == 4) %LUR value
        line([j,j],[1.025,1.05],'Color',colorNonPref,'LineWidth',widthUnR); %LUR line
    elseif (wantedEM(j) == 5) || (wantedEM(j) == 7) %RR value
        line([j,j],[-0.05,0],'Color',colorPref,'LineWidth',widthR); %RR line
    elseif (wantedEM(j) == 6) || (wantedEM(j) == 8) %RUR value
        line([j,j],[-0.05,-0.025],'Color',colorPref,'LineWidth',widthUnR); %RUR line
...
for r = 1:length(correctDir) %going through the correct direction vector
    if (correctDir(r) == 0) %if the correct direction is LEFT...
        line([r,r+1],[1.1,1.1],'Color',colorNonPref,'LineWidth',12); %...plot a box on top in blue
    elseif (correctDir(r) == 1) %if the correct direction is RIGHT...
        line([r,r+1],[1.1,1.1],'Color',colorPref,'LineWidth',12); %...plot a box on the top in pink
    end %ending "if" statement"
end %ending "for" loop
...
y = [mean(minnoleft_percent) mean(minnoright_percent); mean(lownoleft_percent) mean(lownoright_percent); mean(highnoleft_percent) mean(highnoright_percent)]; %performance by block type
a_vert = [std(minnoleft_percent)/sqrt(length(minnoleft_percent)) std(minnoright_percent)/sqrt(length(minnoright_percent)); std(lownoleft_percent)/sqrt(length(lownoleft_percent)) std(lownoright_percent)/sqrt(length(lownoright_percent)); std(highnoleft_percent)/sqrt(length(highnoleft_percent)) std(highnoright_percent)/sqrt(length(highnoright_percent))]; %SEM
errorplus_vert=a_vert'; %positive SEM
errorminus_vert=errorplus_vert; %negative SEM
