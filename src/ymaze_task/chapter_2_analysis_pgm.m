...
for environment = 1:6 %go through all environments doing the streamlined way to pull data out
    fName = get_filename(environment);
    filename = load(fName);
...
for environment = 1:6 %go through all environments doing the streamlined way to pull data out
    if environment == 1 %starting "if" statement to load each individual overlapped
        filename = load([paths.dropPath, 'ymaze/data/NoConfLowVolControl_withDTs.mat']);
    elseif environment == 2 %starting "if" statement to load each individual overlapped
        filename = load([paths.dropPath, 'ymaze/data/LowConfLowVolControl_withDTs.mat']);
    elseif environment == 3 %starting "if" statement to load each individual overlapped
        filename = load([paths.dropPath, 'ymaze/data/HighConfLowVolControl_withDTs.mat']);
    elseif environment == 4 %starting "if" statement to load each individual overlapped
        filename = load([paths.dropPath, 'ymaze/data/NoConfHighVolControl_withDTs.mat']);
    elseif environment == 5 %starting "if" statement to load each individual overlapped
        filename = load([paths.dropPath, 'ymaze/data/LowConfHighVolControl_withDTs.mat']);
    elseif environment == 6 %starting "if" statement to load each individual overlapped
        filename = load([paths.dropPath, 'ymaze/data/HighConfHighVolControl_withDTs.mat']);
...
function fName = get_filename(environment)
paths = get_paths;
    if environment == 1 %starting "if" statement to load each individual overlapped
        fName = [paths.dropPath, 'ymaze/data/NoConfLowVolControl_withDTs.mat'];
    elseif environment == 2 %starting "if" statement to load each individual overlapped
        fName = [paths.dropPath, 'ymaze/data/LowConfLowVolControl_withDTs.mat'];
    elseif environment == 3 %starting "if" statement to load each individual overlapped
        fName = [paths.dropPath, 'ymaze/data/HighConfLowVolControl_withDTs.mat'];
    elseif environment == 4 %starting "if" statement to load each individual overlapped
        fName = [paths.dropPath, 'ymaze/data/NoConfHighVolControl_withDTs.mat'];
    elseif environment == 5 %starting "if" statement to load each individual overlapped
        fName = [paths.dropPath, 'ymaze/data/LowConfHighVolControl_withDTs.mat'];
    elseif environment == 6 %starting "if" statement to load each individual overlapped
        fName = [paths.dropPath, 'ymaze/data/HighConfHighVolControl_withDTs.mat'];
    end %ending "if" statement for environments
end


function [LL_total,Q_total] = fit_Qvalue_V2(choices, rewards, betas, trials) %this is the function that will get us our variables
a = betas(1,1); %first beta is learning rate for rewarded trials
Beta = betas(1,2); %second beta is explore/exploit (higher value = exploitative)
bias = betas(1,3); %third beta is bias
Q_total = NaN(trials,2); %two columns of NaNs for Q values for each trial
LL_total = NaN(trials,1); %one column of NaNs for LL values for each trial
Q_total(1,:) = 0; %fill in first row of Q values with 0s
for I = 1:trials %going through all the trials
    QR = Q_total(I,1); %first column is Q values for RIGHT
    QL = Q_total(I,2); %second column is Q values for LEFT
    prob = 1/(1+exp(-Beta*(QR-QL) + bias)); %calculating the probability of choosing something given our betas/Qs
    if choices(I,1) == 2 %if choice was LEFT...
        LL_total(I,1) = log(1-prob); %calculate new LL
    end %ending "if" statement for choosing LEFT
    if choices(I,1) == 1 %if a choice was RIGHT...
        LL_total(I,1) = log(prob); %calculate new LL
    end %ending "if" statement for choosing RIGHT
    if choices(I,1)==1   %if a choice was RIGHT...
        if rewards(I,1) == 1 %if it was rewarded...
            QR_new = QR + a * (1 - QR);%calculate new RIGHT Q value
        end %ending "if" statement for rewarded RIGHT
        if rewards(I,1) == 0 %if it was unrewarded...
            QR_new = QR + a * (0 - QR); %calculate new RIGHT Q value
        end %ending "if" statement for unrewarded RIGHT
        QL_new = QL; %the Q value for LEFT doesn't change
    end %ending "if" statement for choosing RIGHT
    if choices(I,1)==2  %if the choice was LEFT...
        if rewards(I,1) == 1 %if it was rewarded...
            QL_new = QL + a * (1 - QL); %calculate new LEFT Q value
        end %ending "if" statement for rewarded LEFT
        if rewards(I,1) == 0 %if it was unrewarded...
            QL_new = QL + a * (0 - QL); %calculate new LEFT Q value
        end %ending "if" statement for unrewarded LEFT
        QR_new = QR; %Q value for RIGHT doesn't change
    end %ending "if" statement for LEFT choice
    Q_total(I+1,1) = QR_new; %fill in the overall Q matrix with the new RIGHT Q value for next trial
    Q_total(I+1,2) = QL_new; %fill in overall Q matrix with the new LEFT Q value for next trial   
end %ending "for" loop going through all trials
assignin('base', 'Q_total', Q_total); %this creates the Q value table you've been trying to get! yay! (right = column 1; left = column 2)
end %ending function
