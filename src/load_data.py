...
def load_data(opts, dataType):
    ...
    if dataType == 'behavior':
        ...
        data['bhvName'] = np.concatenate([data_window['Behavior'].iloc[0], data_window['Behavior'].iloc[change_bhv_idx]])
        data['bhvStartTime'] = np.concatenate([0, data_window['Time'].iloc[change_bhv_idx]])
        ...
        spike_clusters = np.load(opts.dataPath + 'spike_clusters.npy')
